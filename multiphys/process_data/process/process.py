...
def read_PHALP_data(VIDEOS_ROOT, data_name, subject, seq_name):
    ...
    if data_name == 'chi3d':
        ...
        phalp_path = f"/home/nugrinovic/code/NEURIPS_2023/slahmr/videos/{data_name}/phalp_light/{seq_name}.pkl"
        phalp_path = replace_slahmr_path(phalp_path)
    try:
...
def slahmr_to_world(pose_aa, trans_orig, betas, res_dict, debug=False):
    ...
    if debug:
        ...
        id = 0
        id = 1
        verts, faces = smpl_to_verts(pose_aa_w_i[id], trans_w_i[id], betas=betas[id, :10])
        save_trimesh(verts[0, 0], faces, f"inspect_out/process_chi3d/{seq_name}/world_{id}.ply")
        verts, faces = smpl_to_verts(pose_aa[id], trans_orig[id], betas=betas[id, :10])
        save_trimesh(verts[0, 0], faces, f"inspect_out/process_chi3d/{seq_name}/cam_{id}.ply")
    return pose_aa_w_i, trans_w_i

def prepare_multiperson_smpl_data(pose_aa, trans, betas, cam, debug=0, seq_name=None, person_id=None, to_floor=False):
    ...
    for n in range(len(pose_aa)):
        ...
        pose_aa_n, trans_n, betas_n = pose_aa[n], trans[n], betas[n]
        if to_floor:
            try:
                trans_n, trans_delta_n = trans_body_to_floor(pose_aa_n, trans_n, betas_n)
            except:
        ...
        if debug:
            # save smpl mesh
            verts, faces = smpl_to_verts(pose_aa_n, trans_n, betas=betas_n[:, :10])
            out_path = f"inspect_out/phalp_process/{seq_name}/phalp_{person_id}_{n:02d}.ply"
        ...
        (_, joints_3d), _ = smpl_to_verts(pose_aa_n, trans_n, betas=betas_n[:, :10], return_joints=True)
        joints_3d_ref = joints_3d[0, 0, :25].detach().numpy()
        # get smpl joints in image space
        jts_img_all.append(project_jts(cam[0], joints_3d_ref))
        pose_all.append(pose_aa_n)
        betas_all.append(betas_n)
        trans_all.append(trans_n)
        joints_3d_all.append(joints_3d)
        trans_delta.append(trans_delta_n)

    pose_aa = np.stack(pose_all)
...
def get_phalp_kpts_simple(data, seq_name, ref_jts, data_name):
    ...
    for n, im_name in enumerate(img_names):
        ...
        if 0:
            ...
            im_path = replace_slahmr_path(im_path)
            img = read_image_PIL(im_path)
            plot_joints_cv2(img, j2d[None], show=True, with_text=False)
    ...
    if 0:
        ...
        make_video_w_skel(vitpose_orig, seq_name, data_name="hi4d")
        make_video_w_pID(vitpose_orig, seq_name, data_name="hi4d")
        make_video_w_skel(op_kpts, seq_name, name="_matched_p2")

    # vit_lens = [len(v) for v in vitpose_orig]
    # max(vit_lens)
    # min(vit_lens)
    return op_kpts


def get_phalp_matched_kpts_simple_all(data, seq_name, ref_jts, data_name, debug=False):
    smpl2op_map = smpl_to_openpose("smpl",
                                   use_hands=False,
                                   use_face=False,
                                   use_face_contour=False,
    ...
    for n, im_name in enumerate(img_names):
        ...
        if debug:
            j2d = np.array(vitpose[0])[:, :2]
            im_path = f"/home/nugrinovic/code/NEURIPS_2023/slahmr/videos/viw/images/{seq_name}/000001.jpg"
            im_path = f"/sailhome/nugrinov/code/CVPR_2024/slahmr_release/slahmr/videos/chi3d/train/s03/images/{seq_name}/000001.jpg"
            # im_path = replace_slahmr_path(im_path)
            img = read_image_PIL(im_path)
            plot_joints_cv2(img, j2d[None], show=True, with_text=False)
            plot_joints_cv2(img, ref_jts[0, None], show=True, with_text=True)
            plot_joints_cv2(img, ref_jts[1, None], show=True, with_text=True)
            img_w_kpts2d = plot_skel_cv2(img, j2d)
            plot(img_w_kpts2d)
            
            j2d_vit = np.array(vitpose)
            img_w_boxes = get_img_w_pID(0, j2d_vit, seq_name, t_id_all=None, data_name="hi4d", name="")
            plot(img_w_boxes)
            
            j2d_ref = np.zeros_like(j2d_vit)
            j2d_ref[:, :, :2] = ref_jts
            j2d_ref[:, :, 2] = j2d_vit[:, :, 2]
            img_w_boxes = get_img_w_pID(0, j2d_ref, seq_name, t_id_all=None, data_name="hi4d", name="")
            plot(img_w_boxes)
    ...
    if debug:
        ...
        img = read_image_PIL(im_path)
        plot_joints_cv2(img, ref_jts_op, show=True, with_text=True)
        plot_joints_cv2(img, op_kpts[0], show=True, with_text=True)
        make_video_w_skel(vitpose_orig, seq_name, data_name="hi4d")
        make_video_w_pID(vitpose_orig, seq_name, t_id_all, data_name="hi4d")
        make_video_w_pID(op_kpts, seq_name, data_name="hi4d", name="_matched")

        img_w_boxes = get_img_w_pID(0, vitpose_orig[0], seq_name, t_id_all=None, data_name="hi4d", name="")
        plot(img_w_boxes)

        img_w_boxes = get_img_w_pID(0, op_kpts[0], seq_name, t_id_all=None, data_name="hi4d", name="")
        plot(img_w_boxes)

        j2d_vit = np.array(vitpose_orig[0])
        j2d_ref = np.zeros_like(j2d_vit)
        j2d_ref[:, :, :2] = ref_jts
        j2d_ref[:, :, 2] = j2d_vit[:, :, 2]
        img_w_boxes = get_img_w_pID(0, j2d_ref, seq_name, t_id_all=None, data_name="hi4d", name="")
