...
from multiphys.metrics.tools import (
    local_align_joints,
    global_align_joints,
    first_align_joints,
)
...
def reorder_joints(gt_seq_joints, njts):
    gt_seq_joints_ = gt_seq_joints.reshape(2, -1, njts, 3)
    gt_p1 = gt_seq_joints_[0]
    gt_p2 = gt_seq_joints_[1]
    gt_seq_joints_ = torch.cat([gt_p1, gt_p2], dim=1)
...
def compute_metrics_mp(res, converter=None, key=None, args=None, n=None, use_wbpos=False, use_smpl_gt=False, debug=False):
    ...
    for name in metric_names:
        ...
        if name == "pampjpe":
            gt_seq_joints_ = reorder_joints(gt_seq_joints, njts)
            res_joints_ = reorder_joints(res_joints, njts)
            pred = local_align_joints(gt_seq_joints_, res_joints_)
            pred = split_joints_per_person(pred, njts)
            target = split_joints_per_person(gt_seq_joints_, njts)
        elif name == "ga_jmse":
        ...
        if name == "pampjpe":
            err_per_fr = 1000 * torch.linalg.norm(target - pred, dim=-1).mean(-1)
            res_dict['pampjpe_per_fr'] = err_per_fr
        elif name == "ga_jmse":
            target_ = target.reshape(2, -1, njts, 3)
            pred_ = pred.reshape(2, -1, njts, 3)
            err_per_fr = 1000 * torch.linalg.norm(target_ - pred_, dim=-1).mean(-1)
            res_dict['ga_jmse_per_fr'] = err_per_fr.T
        elif name == "fa_jmse":
            target_ = target.reshape(2, -1, njts, 3)
            pred_ = pred.reshape(2, -1, njts, 3)
            err_per_fr = 1000 * torch.linalg.norm(target_ - pred_, dim=-1).mean(-1)
            res_dict['fa_jmse_per_fr'] = err_per_fr.T
        ...
        if debug:
            ...
            target_mp = target_mp[::5]
            for n in trange(len(pred_mp)):
                joints_to_skel(pred_mp[n, 0, None], f"{op}_p1/pred/skels_pred_{n:03d}.ply", format=fmt, radius=rad,sphere_rad=sphere_rad, save_jts=get_jts)
                joints_to_skel(pred_mp[n, 1, None], f"{op}_p2/pred/skels_pred_{n:03d}.ply", format=fmt, radius=rad,sphere_rad=sphere_rad, save_jts=get_jts)
                joints_to_skel(target_mp[n, 0, None], f"{op}_p1/gt/skels_gt_{n:03d}.ply", format=fmt, radius=rad,sphere_rad=sphere_rad, save_jts=get_jts)
                joints_to_skel(target_mp[n, 1, None], f"{op}_p2/gt/skels_gt_{n:03d}.ply", format=fmt, radius=rad,sphere_rad=sphere_rad, save_jts=get_jts)
