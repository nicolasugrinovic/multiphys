...
def match_w_hungarian(joints_3d_rel, joints_3d_sla_rel, clip=True):
    ...
    for j3d_rel in joints_3d_rel:
        l2_dist = np.linalg.norm(joints_3d_sla_rel - j3d_rel, axis=-1).mean(1)
...
def load_body_model(batch_size, model_type, gender, device):
    ...
    smpl_path = f"{BASE_DIR}/data/{model_type}/{gender}/model.{ext}"
