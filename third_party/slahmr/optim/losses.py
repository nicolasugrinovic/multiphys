...
class RootLoss(StageLoss):
    ...
    def forward(self, observed_data, pred_data, valid_mask=None):
        ...
        if (
            ...
            debug = False
            if debug:
                from utils.motion_data_utils import get_dataset_image
                from utils.misc import plot_joints_cv2
                idx = 0
                data_name = "hi4d"
                subj_name = "."
                seq_name = "_pair00_1_fight00"
                img = get_dataset_image(idx, data_name, subj_name, seq_name)
                plot_joints_cv2(img, joints2d[0, 0, None].cpu(), with_text=True, sc=2)
                obs_j2d = observed_data["joints2d"]
                confs = obs_j2d[0, 0, :, 2] > 0
                plot_joints_cv2(img, obs_j2d[0, 0, None].cpu(), with_text=True, sc=2)
                plot_joints_cv2(img, joints2d[0, 0, None, confs].cpu(), with_text=True, sc=2)
                plot_joints_cv2(img, obs_j2d[1, 0, None].cpu(), with_text=True, sc=2)
                plot_joints_cv2(img, joints2d[1, 0, None, confs].cpu(), with_text=True, sc=2)
