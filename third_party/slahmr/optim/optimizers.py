...
class StageOptimizer(object):
    ...
    def vis_result(self, res_dir, obs_data, vis=None, num_steps=-1):
        ...
        # self.save_mesh_scenes(scene_dict, res_dir)

    def save_meshes_to_file(self, res_dir, obs_data, num_steps=-1):
        # check which results are saved
        seq_name = obs_data["seq_name"][0]
        res_pre = f"{res_dir}/out_meshes"
        with torch.no_grad():
            pred_dict = self.model.get_optim_result(num_steps=num_steps)
        ...
        verts = scene_dict["geometry"][0][::5]
        verts = torch.cat(verts)
        faces = self.model.body_model.bm.faces
        save_mesh(verts, faces, f"{res_pre}/poses_simu.ply")
        # self.save_mesh_scenes(scene_dict, res_pre)
    ...
    def run(self, obs_data, num_iters, out_dir, vis=None, writer=None, overwrite=False):
        self.cur_step = 0
        ...
        self.save_results(res_dir, seq_name)
        self.vis_result(res_dir, obs_data, vis)
        # self.save_meshes_to_file(res_dir, obs_data)
...
class SMPLOptimizerSimu(StageOptimizer):
    name = "smpl_fit_simu"
    stage = 0
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # param_names = ["trans", "root_orient", "betas", "latent_pose"]
        param_names = ["trans", "root_orient", "latent_pose"]
        if model.opt_scale:
            param_names += ["world_scale"]
...
class SmoothOptimizerParams(SmoothOptimizer):
    name = "smooth_fit"
    stage = 1
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert param_names is not None, "Must specify param_names!!!"
        # if model.opt_scale:
        #     param_names += ["world_scale"]
        # if model.opt_cams:
        #     param_names += ["cam_f", "delta_cam_R"]
...
class MotionOptimizerFreeze(MotionOptimizer):
    ...
    def __init__(self, model, all_loss_weights, opt_cams=False, no_contacts=False, param_names=None, **kwargs):
        assert param_names is not None, "Must specify param_names!!!"
        ...
        # param_names = ["latent_motion", "betas", "trans"]
