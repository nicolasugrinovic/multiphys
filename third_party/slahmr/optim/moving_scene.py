...
from embpose_util.tensor import (
    detach_all,
    scatter_intervals,
    select_intervals,
    get_scatter_mask,
...
class MovingSceneModel(BaseSceneModel):
    ...
    def initialize(self, obs_data, cam_data, param_dict, data_fps):
        ...
        # This following part is not included in the original, added by NUK
        self.set_init_pose(obs_data)

    def set_init_pose(self, obs_data):
    ...
    def pred_params_smpl(self, pred_data):
        pred_data_w = self.get_smpl_data(pred_data['world'])
        pred_data_p = self.get_smpl_data(pred_data['prior'])
        out = {
            'world': pred_data_w,
            'prior': pred_data_p,
        }
    ...
    def rollout_smpl_steps(self, num_steps=-1):
        ...
        # not necessary for EmbPose, but todo restore this if needed
