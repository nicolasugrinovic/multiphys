...
class MultiPeopleDataset(Dataset):
    ...
    def load_data(self, interp_input=True):
        ...
        for i, tid in enumerate(self.track_ids):
            ...
            # load single image smpl predictions (NUK: paths to PHALP results, this is PHALP output but in .json format, the funtion export_phalp_predictions is used to save these)
            pred_paths = [
            ...
            data_out["init_trans"].append(trans_init) # NUK: for phalp, this is just "cam_trans" as read by fnc read_smpl_preds()
