...
class DatasetBatch(data.Dataset):
    def __init__(self, cfg, data_files, seed=0, multiproess=True):
        ...
        for f in data_files:
            ...
            # raw_data contains cam
            processed_data, raw_data = self.post_process_data(processed_data, raw_data)
            [self.data[k].update(v) for k, v in processed_data.items()]
            # data_raw contains original data order
    ...
    def preprocess_data(self, data_file):
        ...
        if self.multiproess:
            ...
            # here all_data joitns2d are still (B, 25, 3), meaning still Openpose (OP)
            # but are processed and changed from 25 --> 12 joints, from OP order to their
