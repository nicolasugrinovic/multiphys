...
class Skeleton:
    ...
    def write_str(
        ...
        tree = self.construct_tree(ref_angles=ref_angles, offset=offset, template_fname=template_fname, num_agent=num_agent)
        if bump_buffer:
    ...
    def construct_tree(
        ...
        # template_fname here is 'assets/mujoco_models/template/humanoid_template.xml'
        tree = parse(template_fname, parser=parser)
        worldbody = tree.getroot().find("worldbody")
        # create worldbody
        self.write_xml_bodynode(self.root, worldbody, offset, ref_angles, num_agent=num_agent)
        ...
        add_agent_name = f"{num_agent}_" if num_agent > 1 else ""

        # create exclude contacts
        c_node = tree.getroot().find("contact")
        for bname1, bname2 in self.exclude_contacts:
            attr = {"body1": add_agent_name+bname1, "body2": add_agent_name+bname2}
    ...
    def write_xml_bodynode(self, bone, parent_node, offset, ref_angles, num_agent=1):
        add_agent_name = f"{num_agent}_" if num_agent > 1 else ""

        attr = dict()
        # NUK: modified by me to add agent name
        attr["name"] = add_agent_name + bone.name
        ...
        if bone.parent is None:
            j_attr = dict()
            # NUK: modified by me to add agent name
            j_attr["name"] = add_agent_name + bone.name
            ...
            for i in range(len(bone.channels)):
                ...
                j_attr = dict()
                # NUK: modified by me to add agent name
                j_attr["name"] = add_agent_name + bone.name + "_" + bone.channels[i]
                j_attr["type"] = "hinge"
                j_attr["pos"] = "{0:.4f} {1:.4f} {2:.4f}".format(*(bone.pos + offset))
        ...
        # NOTE: i don't have to add a name here, this is for grabbing the mesh files
        ...
        for bone_c in bone.child:
            # NUK: modified by me to add agent name
            self.write_xml_bodynode(bone_c, node, offset, ref_angles, num_agent)
