...
class PolicyGaussian(Policy):
    ...
    def forward(self, x):
        x = self.net(x) # input is (657) / output is (1, 512)
        action_mean = self.action_mean(x) # output is (1, 315)
        action_log_std = self.action_log_std.expand_as(action_mean)
        action_std = torch.exp(action_log_std)
        return DiagGaussian(action_mean, action_std) # inits a normal dist. object with mean and std
