...
class Robot:
    ...
    def __init__(self, cfg, data_dir="data/smpl", masterfoot=False, num_agent=1):
        self.bodies = []
        self.weight = 0
        self.height = 0
        self.cfg = cfg
        ...
        # SceneRobot.load_from_skeleton ? goes to uhc/smpllib/smpl_robot.py '': load_from_skeleton()
        self.load_from_skeleton(num_agent=num_agent)
    ...
    def load_from_skeleton(
        ...
        if self.mesh:
            ...
            (
                ...
                contype,
                conaffinity,
            ) = (smpl_parser.get_mesh_offsets(betas=self.beta, flatfoot=self.flatfoot) if self.smpl_model != "smplx"
            ...
            # hull_dict keys are the joint names, each contains a scipy hull object
        ...
        self.tree = parse(
            BytesIO(
                self.skeleton.write_str(bump_buffer=True, num_agent=num_agent)
            ),
            parser=parser,
        )
        ...
        add_agent_name = f"{num_agent}" if num_agent > 1 else ""
        ...
        SubElement(
            ...
            "exclude",
            {
                "name": "add01" + add_agent_name,
                "body1": "L_Shoulder" + add_agent_name,
                "body2": "Chest" + add_agent_name
            },
        )
        SubElement(
            contact_node,
            "exclude",
            {
                "name": "add02" + add_agent_name,
                "body1": "R_Shoulder" + add_agent_name,
                "body2": "Chest" + add_agent_name
            },
        )
