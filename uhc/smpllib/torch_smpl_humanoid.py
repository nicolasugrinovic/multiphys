...
class Humanoid:
    ...
    def update_model(self, model, n_agent=0):
        # called from HumanoidKinEnvRes.load_context()
        # model is the mujoco model
        # NOTE: this only takes info account offsets info from agent 1.
        # Should it also take info from agent 2?
        jts_n = 24
        self.body_name = model.body_names[1:jts_n + 1]

        # not necessary to change body_names but DO change offsets!!

        # if n_agent == 0:
        #     self.body_name = model.body_names[1:jts_n + 1]
        # elif n_agent == 1:
        #     self.body_name = model.body_names[-jts_n:]
        # else:
        #     raise NotImplementedError

        # Change offsets to enable two agents opperation
        if n_agent == 0:
            ...
            offsets = model.body_pos[-len(self.body_name):]
            i_offsets = model.body_ipos[-len(self.body_name):]
            # Do this because otherwise there is an error in _compute_metadata(self) --> needs to have
            # indices from 0 to 24
            # parents = model.body_parentid[-len(self.body_name):] - 1
            parents = model.body_parentid[1:len(self.body_name) + 1] - 1
        else:
            raise NotImplementedError


        # NUK a bit hacky to filter only agent 1 body parts
        # todo make this more general, perhaps have annother humanoid instance for agent 2
        # self.body_name = tuple([c for c in self.body_name if '2' not in c])
        # if n_agent == 0:
        #     self.body_name = model.body_names[1:jts_n+1]
        #     offsets = model.body_pos[1:len(self.body_name) + 1]
        #     i_offsets = model.body_ipos[1:len(self.body_name) + 1]
        #     parents = model.body_parentid[1:len(self.body_name) + 1] - 1
        # elif n_agent == 1:
        #     self.body_name = model.body_names[jts_n+1:2*jts_n+1]
        #     offsets = model.body_pos[len(self.body_name) + 1:]
        #     i_offsets = model.body_ipos[len(self.body_name) + 1:]
        #     parents = model.body_parentid[len(self.body_name) + 1:] - 1
        # else:
        #     raise NotImplementedError
    ...
    def qpos_fk(self, qpos, select_joints=None, fix_quat=False, to_numpy=True, full_return=True):
        ...
        Rotations are represented in euler angles, root orientation is represented in quaternion.
        ...
        # filter here only agent 1 joints for now, later filter per agent
        # NUK: added by me. This function is called pero agent, because self.body_name is used to get the len and
        # also to get the single agent jts locations, the following works ok:
        self.body_name = tuple([c for c in self.body_name if '2' not in c])
        ...
        # there is a problem here when adding the second agent
        # body_quats is ([23, 4]) and J here is 47
        body_quats = body_quats.reshape(B, J, 4)
        # body_quats ([1, 23, 4]), root_rotations ([1, 4]), root_positions ([1, 3])
        # with the hack this one works for now but update!

        # OFFSETS are USED here! --> so this need to be multi-agent
        ...
        if full_return:
            ...
            return_dic = {
                "qpos": qpos,
                "qvel": qvel,
                "wbpos": wbody_pos.reshape(batch_size, -1), ### this
        ...
        # print(self._offsets),  print(self._i_offsets)
        if to_numpy:
    ...
    def get_ee_pos(self, body_xpos, root_q, transform):
        #####
        keys = list(self.model._body_name2id.keys())
        agent_id = 2 if "Pelvis2" in keys else 1
        if agent_id == 2:
            ee_name = [c+"2" for c in SMPL_EE_NAMES]
        else:
    ...
    def forward_kinematics_batch(self, rotations, root_rotations, root_positions):
        ...
        device, dtype = root_rotations.device, root_rotations.dtype
        B = rotations.size()[0] # B  is 1
        J = self._offsets.shape[0] # J  is 24
        # print(self._offsets)
        positions_world = [] # this is wbody_pos
        positions_world_com = []
        rotations_world = []
        expanded_offsets = ( # this affests wbody_pos
            self._offsets.expand(B, J, self._offsets.shape[1]).to(device).type(dtype)
        )
        # J_i = self._i_offsets.shape[0]
        expanded_i_offsets = ( # self._i_offsets is ([24, 3])
        ...
        for i in range(J):
            if self._parents[i] == -1:
                positions_world.append(root_positions) # wbody_pos
                ...
                jpos = (
                    quat_mul_vec_batch(
                        rotations_world[self._parents[i]], expanded_offsets[:, i, :] # wbody_pos
                    )
                    + positions_world[self._parents[i]] # wbody_pos
                )
                ...
                positions_world.append(jpos) # wbody_pos
        ...
        return (# these are wbody_pos, body_com, wbody_quat
