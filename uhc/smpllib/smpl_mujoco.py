...
class SMPLConverter:
    ...
    def __init__(self, model, new_model, smpl_model="smpl"):
        ...
        self.new_qpos_addr = get_body_qposaddr(new_model) # for one agent is (24) for two agents is (48)
        self.new_qvel_addr = get_body_qveladdr(new_model) # for one agent is (24) for two agents is (48)

        self.smpl_joint_names = list(self.smpl_qpos_addr.keys())
        # for multi maybe only change this
        self.new_joint_names = list(self.new_qpos_addr.keys()) # this is normally (24)
        # NUK: very hacky. todo multi: change this
        self.new_joint_names = [c for c in self.new_joint_names if '2' not in c]
...
def smpl_to_qpose(
    ...
    differentiable 
    """
    if trans is None: # to compute init_qpos, trans is given
    ...
    names_list = list(get_body_qposaddr(mj_model).keys()) # when there's 2 agents this is 48, else 24
    # NUK: addedfilter number in list:
    # names_list = [q[:-1] for q in names_list if '2' in q]
    smpl_2_mujoco = [joint_names.index(q) for q in names_list if q in joint_names]
    ...
    if count_offset:
        # todo multi: this is fixed for one agent, should make it work for multiple agents
...
def smpl_to_qpose_multi(
    ...
    euler_order="ZYX",
    model="smpl",
    # mujoco_body_order=None,
    agent_id=None,
):
    """
    pose: is SMPL pose (batch_size x 72)
    Expect pose to be batch_size x 72
    trans to be batch_size x 3
    differentiable 
    """
    assert agent_id is not None, "*** 'agent_id' must be specified in smpl_to_qpose_multi"

    if trans is None: # NOT here
        trans = np.zeros((pose.shape[0], 3))
        trans[:, 2] = 0.91437225
    if normalize: # NOT here
        pose, trans = normalize_smpl_pose(pose, trans, random_root=random_root)
    ...
    num_joints = len(joint_names) # 24
    num_angles = num_joints * 3 # 72
    # this one does not matter much to change to multi-person because I pass one per agent
    # smpl_2_mujoco = [joint_names.index(q) for q in mujoco_body_order if q in joint_names]
    names_list = list(get_body_qposaddr(mj_model).keys()) # 48 (2 agents), else 24
    smpl_2_mujoco = [joint_names.index(q) for q in names_list if q in joint_names]
    # agent1: [0, 1, 4, 7, 10, 2, 5, 8, 11, 3, 6, 9, 12, 15, 13, 16, 18, 20, 22, 14, 17, 19, 21, 23]
    pose = pose.reshape(-1, num_angles) # (1, 72)

    curr_pose_mat = angle_axis_to_rotation_matrix(pose.reshape(-1, 3)).reshape(pose.shape[0], -1, 4, 4) # (1, 24, 4, 4)
    ...
    if use_quat:
        ...
        curr_spose = curr_spose.as_euler(euler_order, degrees=False).reshape(curr_pose_mat.shape[0], -1) # (1, 72)
    # NUK: curr_spose is an SMPL pose, with 72 angles, we need to convert it to mujoco pose
    # it is ok to do this independently per agent because we will concat them later outside in the correct order
    curr_spose = curr_spose.reshape(-1, num_joints, 4 if use_quat else 3)[:, smpl_2_mujoco, :].reshape(-1, num_angles)
    if use_quat:
        curr_qpos = np.concatenate([trans, curr_spose], axis=1)
    else: # here

        root_quat = rotation_matrix_to_quaternion(curr_pose_mat[:, 0, :3, :]) # (1, 4)
        curr_qpos = np.concatenate((trans, root_quat, curr_spose[:, 3:]), axis=1) # (1, 76)

    if count_offset:
        # this one does matter for multi-person!
        idx = 1 + agent_id * 24
        curr_qpos[:, :3] = trans + mj_model.body_pos[idx]
...
def smpl_to_qpose_torch(
    ...
    # todo multi: potential bug here!!! the offset has to be different for each agent
    if count_offset: # is True
        curr_qpos[:, :3] = trans + torch.from_numpy(mj_model.body_pos[1]).to(root_quat)

    return curr_qpos


def smpl_to_qpose_torch_multi(
    pose,
    mj_model,
    trans=None,
    normalize=False,
    ...
    # multi: potential bug here!!! the offset has to be different for each agent
    # if count_offset: # is True
    #     curr_qpos[:, :3] = trans + torch.from_numpy(mj_model.body_pos[1]).to(root_quat)
    # thsi should be correct for multi
    if count_offset:
        # this one does matter for multi-person!
        idx = 1 + agent_id * 24
        curr_qpos[:, :3] = trans + torch.from_numpy(mj_model.body_pos[idx]).to(root_quat)

    return curr_qpos


def qpos_to_smpl(qpos, mj_model, smpl_model="smpl", ret_body_pose=False):
    #
    body_qposaddr = get_body_qposaddr(mj_model)
    batch_size = qpos.shape[0]
    # todo multi: potential but here also!! the root should be different for each agent
    trans = qpos[:, :3] - mj_model.body_pos[1]# (2, 3)
    smpl_bones_to_use = (SMPL_BONE_ORDER_NAMES if smpl_model == "smpl" else SMPLH_BONE_ORDER_NAMES)
    pose = np.zeros([batch_size, len(smpl_bones_to_use), 3]) # (2, 24, 3)
    ...
    if ret_body_pose:
        return pose, trans, mj_model.body_pos[1]
    return pose, trans


def qpos_to_smpl_multi(qpos, mj_model, smpl_model="smpl", agent_id=None):
    #
    assert agent_id is not None, "*** 'agent_id' must be specified in qpos_to_smpl_multi"

    body_qposaddr = get_body_qposaddr(mj_model)
    batch_size = qpos.shape[0]
    # todo multi: potential but here also!! the root should be different for each agent
    idx = 1 + agent_id * 24
    trans = qpos[:, :3] - mj_model.body_pos[idx]# (2, 3)
    ####
    smpl_bones_to_use = (SMPL_BONE_ORDER_NAMES if smpl_model == "smpl" else SMPLH_BONE_ORDER_NAMES)
    pose = np.zeros([batch_size, len(smpl_bones_to_use), 3]) # (2, 24, 3)
    for ind1, bone_name in enumerate(smpl_bones_to_use):
