...
class Humanoid_Batch:
    ...
    def update_model(self, betas, gender):
        ...
        if len(betas_f) > 0:
            _, _, _, _, joint_offsets_f, *_ = self.smpl_parser_f.get_mesh_offsets_batch(betas=betas_f[:, :10])

        betas_n = betas[gender == 0]
        if len(betas_n) > 0:
            _, _, _, _, joint_offsets_n, *_ = self.smpl_parser_n.get_mesh_offsets_batch(betas=betas_n[:, :10])

        betas_m = betas[gender == 1]
        if len(betas_m) > 0:
            _, _, _, _, joint_offsets_m, *_ = self.smpl_parser_m.get_mesh_offsets_batch(betas=betas_m[:, :10])
    ...
    def proj2d(self, wbpos, return_cam_3d=False, data_name=None):
        ######################## 3D to 2D predicted joints projection ########################
        # here the 3D predicted kinematic pose is projected to 2D joints
        # wbpos in mujoco
        self.proj_counter += 1

        pred_joints3d = wbpos.squeeze()[self.MUJOCO_2_SMPL][self.smpl2op_partial][None, ]

        # if self.proj_counter==1 or self.proj_counter==24:
        #     print("camera params used are:")
        ...
        #         R = R.T
        #         tr = -np.matmul(R, tr)
        # pred_joints3d is (1, 14, 3)
        # if data_name == "prox" or data_name is None:

        pred_joints3d_cam = pred_joints3d @ self.full_R.T + self.full_t
        pred_joints2d = pred_joints3d_cam @ (self.K.T)
        z = pred_joints2d[:, :, 2:]
        pred_joints2d = pred_joints2d[:, :, :2] / z

        #     pred_joints2d_or = pred_joints2d
        # elif data_name == "chi3d":
        #     if len(self.full_t.shape)==2:
        #         pred_joints3d_cam = ((self.full_R @ pred_joints3d[0].T).T + self.full_t)[None]
        #     elif len(self.full_t.shape)==1:
        #         pred_joints3d_cam = ((self.full_R @ pred_joints3d[0].T).T + self.full_t[None])[None]
        #     else:
        #         raise ValueError("full_t has wrong shape")
        #     pred2d = pred_joints3d_cam / pred_joints3d_cam[:, :, 2:]
        #     pred2d = pred2d[0]
        #     pred_joints2d = (self.K @ pred2d.T).T
        #     pred_joints2d = pred_joints2d[:, :2][None]
        #     pred_joints2d_modif = pred_joints2d
            # pred_joints2d_or == pred_joints2d_modif this is very similar! so the way the camera
            # is applied it not the problem, It may go more with the resolution and the fact
            # self.img_h, self.img_w change

        if 0:
            from utils.misc import save_pointcloud
            inspect_path = f"inspect_out/{data_name}/projection/"
            save_pointcloud(pred_joints3d[0], inspect_path + f"pred_joints3d.ply")
            save_pointcloud(pred_joints3d_cam[0], inspect_path + f"pred_joints3d_cam.ply")
            save_pointcloud(pred_joints3d_cam[0], inspect_path + f"pred_joints3d_cam_w_mult.ply")
            save_pointcloud(pred_joints3d_cam[0], inspect_path + f"pred_joints3d_cam_setp.ply")
            save_pointcloud(pred_joints3d[0], inspect_path + f"pred_joints3d_step.ply")
            from utils.misc import plot_joints_cv2
            # black_img = np.zeros((self.img_h, self.img_w, 3))
            black_img = np.zeros((900, 900, 3), dtype=np.uint8)
            # pred_joints2d_ = np.array(pred_joints2d)
            plot_joints_cv2(black_img, pred_joints2d, show=True)
        # print(f"projection counter: {self.proj_counter}")
        pred_joints2d = smpl_op_to_op(pred_joints2d)

        if return_cam_3d:
            return pred_joints2d, pred_joints3d_cam
        else:
    ...
    def proj_2d_loss(self, input_vec, ord=2, normalize = True):
        ...
        if 0:
            ...
            img = plot_joints_cv2(black, pred_joints2d[0, None], show=True, with_text=True, sc=3)
            img = plot_joints_cv2(black, gt_2d_joints[0, None], show=True, with_text=True, sc=3)
