...
class HumanoidEnv(mujoco_env.MujocoEnv):
    ...
    def load_models(self):
        ...
        self.torque_lim = self.converter.get_new_torque_limit() * self.cc_cfg.get("tq_mul", 1) # for one agent (69, )
        self.set_action_spaces()
    ...
    def set_action_spaces(self):
        # todo multi: implement this in the multi env and override this method
        cfg = self.cc_cfg
        self.vf_dim = 0
        self.meta_pd_dim = 0
        # if I add an agent as I am adding now to self.tree these two double in size
        self.ndof = self.model.actuator_ctrlrange.shape[0] # w/ 1 agent this is (69, 2), w/ 2 agents this is (138, 2)

        # NUK: hacky way for now, todo modify soon.
        # make general, o sea, con 138 dof porque esto luego controla la simu
        # but this is alright because it uses this in parts where I have diveded the operation in a for loop to be
        # per agent
        one_agent_dim = 69 # hacer general
        self.ndof = one_agent_dim
        body_id_list = self.model.geom_bodyid.tolist() # 49 for 2 agents
        ...
        if cfg.residual_force:
            if cfg.residual_force_mode == "implicit":
                ...
                if cfg.residual_force_bodies == "all":
                    # self.num_agent is defined in the upper class, not here
                    self.vf_bodies = SMPL_BONE_ORDER_NAMES # list of len 24
                else:
                    self.vf_bodies = cfg.residual_force_bodies
                # list of len 24, for one agent self.model._body_name2id is of len 25 (includes 'world')
                # self.vf_geoms is [1, 2, 6, 10, 3, 7, 11, 4, 8, 12, 5, 9, 13, 15, 20, 14, 16, 21, 17, 22, 18, 23, 19, 24]
                self.vf_geoms = [body_id_list.index(self.model._body_name2id[body]) for body in self.vf_bodies]
                self.body_vf_dim = 6 + cfg.residual_force_torque * 3 # is 9
                # vf_dim is 216
                self.vf_dim = (self.body_vf_dim * len(self.vf_bodies) * cfg.get("residual_force_bodies_num", 1))

        if cfg.meta_pd:
            self.meta_pd_dim = 2 * 15 # is 30
    ...
    def get_full_obs_v2(self, delta_t=0):
        ...
        curr_root_quat = self.remove_base_rot(qpos[3:7]) # shape (4,)
        hq = get_heading_q(curr_root_quat) # is this heading quaternion? shape (4,)
        obs.append(hq)  # obs: heading (4,)
        # ZL : why use heading????. Should remove this...

        ######## target body pose #########
        target_body_qpos = self.get_expert_qpos(delta_t=1 + delta_t)  # target body pose (76, )
    ...
    def get_body_quat(self):
        qpos = self.get_humanoid_qpos() # this is (152,) for 2 agents
        if self.use_quat:
            body_quat = [qpos[3:7]]
            for body in self.model.body_names[1:self.body_lim]:
                # if body == "Pelvis" or body == "2_Pelvis" or not body in self.body_qposaddr:
                if body == "Pelvis" or body == "Pelvis_2" or not body in self.body_qposaddr:
                    if body == "Pelvis_2":
                        start, end = self.body_qposaddr[body]
                        body_quat.append(qpos[start+3:end])
                    continue
                start, end = self.body_qposaddr[body]
            ...
            body_quat = [qpos[3:7]] # for pelvis, because it is already in quaternion
            # ignore body_names[0] as it is world
            for body in self.model.body_names[1:self.body_lim]:
                # ignore Pelvis, it was areaady added above, but add Pelvis_2
                if body == "Pelvis" or body == "Pelvis_2" or not body in self.body_qposaddr:
                    if body == "Pelvis_2":
                        start, end = self.body_qposaddr[body]
                        body_quat.append(qpos[start+3:end])
            ...
            # this should contain the 2 pelvis and the rest of the joints in quat: 48*4 = 192, but pelvis_2 is missing
            body_quat = np.concatenate(body_quat)
        return body_quat
    ...
    def compute_torque(self, ctrl, i_iter=0): # i_iter is the number of skip frames
        cfg = self.cc_cfg
        dt = self.model.opt.timestep
        ctrl_joint = ctrl[:self.ndof] # ctrl_joint (69, ), but ctrl is (315, )
        qpos = self.get_humanoid_qpos() # (76, )
        qvel = self.get_humanoid_qvel() # (75, )

        if self.cc_cfg.action_v == 1:
            base_pos = self.get_expert_kin_pose(delta_t=1)  # base_pos (69, ) # should use the target pose instead of the current pose
        ...
        if cfg.meta_pd: # i_iter=number of skip frames
            # for one agent this is:
            # self.ndof = 69, self.vf_dim = 216, self.meta_pd_dim = 30, self.sim_iter = 15
            meta_pds = ctrl[(self.ndof + self.vf_dim):(self.ndof + self.vf_dim + self.meta_pd_dim)] # (30, )
            curr_jkp = self.jkp.copy() * np.clip((meta_pds[i_iter] + 1), 0, 10) # (69, )
            curr_jkd = self.jkd.copy() * np.clip((meta_pds[i_iter + self.sim_iter] + 1), 0, 10) # (69, )
            # if flags.debug:
        ...
        qvel_err = qvel
        q_accel = self.compute_desired_accel(qpos_err, qvel_err, k_p, k_d) # (75, )
        qvel_err += q_accel * dt
        torque = -curr_jkp * qpos_err[6:] - curr_jkd * qvel_err[6:] # (69, )
        return torque
    ...
    def rfc_explicit(self, vf):
        ...
        for i, body in enumerate(self.vf_bodies):
            ...
            geom_id = self.vf_geoms[i] # self.vf_geoms list of 24
            # self.data.contact tuple (500)
            for contact in self.data.contact[:self.data.ncon]: # self.data.ncon for single agent is 5
            ...
            if not (residual_contact_only and not has_contact):
                for idx in range(num_each_body):
                    ...
                    mjf.mj_applyFT(
                        ...
                        force, # force is (3,)
                        torque, # torque is (3,)
                        contact_point, # contact_point is (3,)
                        body_id, # body_id is int
                        qfrc, # qfrc is (75,)
                    )
        ...
        if 0:
            ...
            inspect_path = "inspect_out/rfc_explicit_single/"
            qpos = self.data.qpos
            from_qpos_to_verts_save(qpos, self, inspect_path, out_fname="qpos_normal.ply")
        ...
        self.data.qfrc_applied[:] = qfrc # qfrc_applied is (75,)
        # print(qfrc)
    """ RFC-Implicit """
    ...
    def do_simulation(self, action, n_frames):
        ...
        for i in range(n_frames):
            # n_frames is the skip frames
            if cfg.action_type == "position":
                torque = self.compute_torque(ctrl, i_iter=i) # for two agents: (138, )
            ...
            # NUK very hacky. todo change
            # self.data.ctrl[:] = torque
            self.data.ctrl[:69] = torque # data.ctrl is (69, ), print(torque)
            """ Residual Force Control (RFC) """
            if cfg.residual_force:
                # it ENTERS HERE
                vf = ctrl[(self.ndof):(self.ndof + self.vf_dim)].copy() # vf is (216, )
                if cfg.residual_force_mode == "implicit":
                    self.rfc_implicit(vf)
                else:
                    # in default eval_scene it comes here
                    self.rfc_explicit(vf) # print(vf)
            # if flags.debug:
            #     self.data.qpos[: self.qpos_lim] = self.get_expert_qpos(
            #         delta_t=-1
            #     )  # debug
    ...
    def reset_model(self):
        ...
        # init_pose[0] += 0.5  # PHC-metrics
        # init_pose[2] = 0.3  # PHC-metrics
        # init_pose[3:7] = np.array([0, 0, 0, 1]) # PHC-metrics
        self.set_state(init_pose, init_vel)
