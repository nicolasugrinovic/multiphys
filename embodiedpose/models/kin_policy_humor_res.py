...
class KinPolicyHumorRes(KinPolicy):
    ...
    """
    NUK: this is the kinematic policy network
    NOTE: the kinematic policy should compute the residual 3D movement, i.e, the state velocity
                    ...
                    # that we care about: trans, ori, and pose velos, these are applied to the pose
    ...
    def __init__(self, cfg, data_sample, device, dtype, agent, mode="train"):
        ...
        self.to(device)
        self.state_dim = state_dim = self.kin_net.state_dim # 6455
        self.action_dim = action_dim = self.kin_net.action_dim # 114
        self.use_tcn = self.kin_net.use_tcn # True
        self.mode = mode # test
    ...
    def forward(self, all_state, gather=True):
        # all_state.shape # ([1, 6455])
        if self.mode == "test" or not self.kin_net.use_rnn:
            action_mean = self.get_action(all_state) # gets action from kin_net
    ...
    def init_context(self, data_dict, random_cam=False):
        ...
        ar_context = self.kin_net.init_states(data_dict, random_cam=random_cam) # KinNetHumorRes
        self.reset() # this does nothing
        if self.kin_net.use_rnn:
            self.reset_rnn(1) # initializes the RNN state
        return ar_context
