...
class UHMModel(nn.Module):
    def __init__(
        ...
        if model_data_config.find('contacts') >= 0:
        ...
        # print(
        #     'Using SMPL joints rather than regressed joints as input at each step for
        #     roll out and scheduled sampling...'
        # )
    ...
    def step_action(self, past_in, action, output_delta=True):
        ...
        if self.output_delta:
            ...
            if self.pred_contacts:
                decode_out_dim_list = decode_out_dim_list[:
                                                          -1]  # do contacts separately
            # for loop over ['trans', 'root_orient', 'pose_body']
            for in_dim_idx, out_dim_idx, data_name in zip(
                    self.input_dim_list, decode_out_dim_list,
                    self.out_data_names):
                in_eidx = in_sidx + in_dim_idx
    ...
    def step_state(self,
        ...
        B, device = past_global_in_dict["trans"].shape[0], past_global_in_dict["trans"].device
        # get world2aligned rot and translation
        ...
        past_in_dict = self.apply_world2local_trans(world2aligned_trans,
                                                    ...
                                                    invert=False)
        past_in = []
        # data_names --> ['trans', 'root_orient', 'pose_body', 'joints', 'root_orient_vel', 'joints_vel']
        for k in self.data_names:
            past_in.append(past_in_dict[k].reshape(B, -1))
        past_in = torch.cat(past_in, axis=1)

        ##### HUMOR prdiction step (?? think not)#######################################
        # To compute the motion prior loss
        # pm, pv = self.prior(past_in.double())
        # this step_action actually only applies the predicted residuals from the kinematic policy
        # to the pose
        ...
        if 0:
            ...
            inspect_path = f"inspect_out/sim_loop/chi3d/motion_prior/"
            trans = past_in_dict["trans"] # shape (1, 1, 3)
            root_orient = past_in_dict["root_orient"] # shape (1, 1, 9)
            pose_body = past_in_dict["pose_body"] # shape (1, 1, 21, 3, 3)
            joints = past_in_dict["joints"] # shape (1, 1, 66)
            prev_joints = joints.reshape([22, 3])
            save_pointcloud(prev_joints, inspect_path + f"prev_joints.ply")

            next_trans = next_out["trans"]  # shape (1, 1, 3)
            next_ori = next_out["root_orient"]  # shape (1, 1, 9)
            next_body = next_out["pose_body"]  # shape (1, 1, 189)
            body_pose_aa = mat2aa(next_body.reshape(21, 3, 3)).reshape(1, 63)
            root_aa = mat2aa(next_ori.reshape(1, 3, 3)).reshape(1, 3)
            next_pose_aa = torch.cat([root_aa, body_pose_aa, torch.zeros(1, 6).to(root_aa)], dim=1)  # (1, 72)
            next_verts, faces = smpl_to_verts(next_pose_aa, next_trans[0])
            save_trimesh(next_verts[0,0], faces, inspect_path+"next_verts.ply")
