...
import numpy as np
import math
from uhc.khrylib.utils import to_device
...
from uhc.utils.torch_geometry_transforms import (rotation_matrix_to_angle_axis as mat2aa)
...
from embodiedpose.models.kin_tcn import TemporalModelOptimized1f
from uhc.utils.torch_ext import isNpArray

from utils.misc import read_pickle

FPS = 30
PROX_SCENE = ['BasementSittingBooth', 'MPH11', 'MPH112', 'MPH16', 'MPH1Library', 'MPH8',
...
class KinNetHumorRes(KinNetBase):
    # Kin net humor res
    # important class and funcion, initializes a lot of things
    def __init__(self, cfg, data_sample, device, dtype, agent, mode="train"):
        ...
        self.motion_prior = UHMModel(in_rot_rep="mat", out_rot_rep=self.model_specs.get("out_rot_rep", "aa"),
                                     latent_size=24, model_data_config="smpl+joints", steps_in=1, use_gn=False)

        to_device(device, self.motion_prior)
        # Freezes weights of motion prior (is this HUMOR?, prob not, down below is HUMOR)
        ...
        # This is actually HUMOR model being loaded here
        self.humor_models = [HumorModel(in_rot_rep="mat", out_rot_rep="aa", latent_size=48,
                                        model_data_config="smpl+joints+contacts", steps_in=1,
        ...
        self.smpl2op_map = smpl_to_openpose(self.motion_prior.bm_dict['neutral'].model_type, use_hands=False,
                                            use_face=False, use_face_contour=False, openpose_format='coco25')
        ...
        # the rest inits the network layers
        if self.use_rnn:
    ...
    def get_dim(self, data_dict):
        ...
        self.is_root_obs = state # (1, 6455)
        self.state_dim = state.shape[-1] # int 6455
        self.action_dim = np.sum(self.motion_prior.output_dim_list) # 69
        self.body_action_dim = np.sum(self.motion_prior.output_dim_list) # 69

        if self.use_tcn:
            self.action_dim += 14 * 3 + 3 # updated to 114

        self.context_dim = self.get_context_dim(data_dict) # 0, set to hard value
    ...
    def get_obs(self, data_dict, t):
        ...
        obs = np.concatenate(obs)[None,] # (1, 6455)
    ...
    def sample_valid_cam_x_y(self, cam_params, trans, offset):
        ...
        frame_ptrs = ptr * z
        x, y = np.random.uniform(np.min(frame_ptrs[:, 0]), np.max(frame_ptrs[:, 0])), \
               np.random.uniform(np.min(frame_ptrs[:, 1]), np.max(frame_ptrs[:, 2]))

        new_ptr = (np.array([[x, y, z]]) - full_t).dot(full_R)
        x, y = new_ptr[0, 0], new_ptr[0, 1]
        # x, y = new_ptr[0], new_ptr[1]
        new_trans[:, :, :2] += np.array([x, y]) - new_trans[:, 0:1, :2]

        trans_cam = ((new_trans + offset).dot(full_R.T) + full_t)
        # trans_cam = ((new_trans + offset[None, None])[0].dot(full_R.T) + full_t)
        ...
        while np.isnan(root_2d).any() or np.sum(root_2d < 0) > 0 or np.sum(trans_cam[..., 2] < 1) > 0 or np.sum(root_2d[..., 0] > 2200) or np.sum(root_2d[..., 1] > 1200):  # should be at least 1.5 m from the camera and within the camera frame
            ...
            if num_trys > 10:
                ...
                self.load_camera_params(self.scene_name, self.seq_name)
    ...
    def init_states(self, data_dict, random_cam=False):
        ...
        if random_cam:
            ...
            pre_body = self.motion_prior.bm_dict[gender](pose_body=pose_aa_empty[:, 1:22].float().reshape(1, -1),
                                                         pose_hand=None,
            ...
            self.scene_name = np.random.choice(PROX_SCENE)
            # this should input the seq name for chi3d
            self.load_camera_params(self.scene_name, data_dict['seq_name'])
            ...
            if "obj_info" in data_dict:
                ...
                if data_dict['load_scene']:
                    ######################################## Scene loading ########################################
                    # this loads the scene from the files containing primitive shapes information for each PROX scene
                    ...
                    while not torch.min(dist) > 0.2:
                        ...
                        x, y, heading_change = self.sample_valid_cam_x_y(self.cam_params, curr_trans,
                                                                         offset.cpu().numpy())
                        dist = get_sdf(scene_sdfs=scene_sdfs, points=torch.tensor([[x, y, 0.5], [x, y, 0],
                                                                                   [x, y, 1], [x, y, 1.5],
                                                                                   [x, y, 0.3], [x, y, 0.7]]), topk=1)

                heading_change = torch.from_numpy(heading_change).to(device).type(self.dtype)
                new_root = torch.matmul(heading_change, data_dict['root_orient'].reshape(-1, 3, 3))

                ## Offset strikes again!!!
                data_dict['trans'] = torch.matmul(data_dict['trans'] + offset, heading_change.T) - offset  
                data_dict['pose_aa'][:, :, :3] = tR.matrix_to_axis_angle(new_root)
                data_dict['root_orient'] = new_root.reshape(B, T, 9)
            ...
            if not 'cam' in data_dict:
                ...
                raise Exception()
            if isinstance(data_dict['cam'], list):
                self.scene_name = data_dict['cam'][0]['scene_name']
                self.cam_params = data_dict['cam'][0]
            else:
        ...
        if not "wbpos_cam" in data_dict:
            ...
            # take the 3D keypoints from world to camera coordinates
            wbpos_cam = wbpos.float() @ full_R.T + full_t
            data_dict['wbpos_cam'] = wbpos_cam

            root_orient = data_dict['root_orient'].float().reshape(-1, 3, 3)
            data_dict['root_cam_rot'] = tR.matrix_to_axis_angle(torch.matmul(root_orient,
    ...
    def load_camera_params(self, scene_name, seq_name):
        ...
        R = R.T
        tr = -np.matmul(R, tr)
        if self.cfg.ignore_align:
            full_R = R
            full_t = tr
        else:
            with open(f'{cam_path}/alignment/{scene_name}.npz', 'rb') as f:
                aRt = np.load(f)
                aR = aRt['R']
                atr = aRt['t']
                aR = aR.T
